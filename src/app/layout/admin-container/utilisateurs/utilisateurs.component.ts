...
export class UtilisateursComponent implements OnInit {
  ...
  listPorifls :  Array<any> = []; // Pour recuperer la lsite des profils
  ...
  getItem() {
    ...
    this.busyGet = this.restService.post('users/getByCriteria', request)
      .subscribe(
        (res: any) => {
          console.log(res);
          if (!res['error']) {
  ...
  getProfil() {
    ...
    this.busyGet = this.restService.post('role/getByCriteria', request)
      .subscribe(
        (res: any) => {
          console.log(res);
          if (!res['error']) {
            this.listPorifls = res?.items;
          }
        },
        err => {
          this.notificationService.showNotificationMessage("error", this.notificationService.defaultMessage());
  ...
  confirmSaveItem(item : any) {
    ...
    if(!item.nom) {
      this.notificationService.showNotificationMessage("error","Veuillez renseigner le nom de l'utilisateur !");
      return;
    }

    if(!item.prenoms) {
      this.notificationService.showNotificationMessage("error","Veuillez renseigner le prénom de l'utilisateur !");
      return;
    }

    if(!item.login) {
      this.notificationService.showNotificationMessage("error","Veuillez renseigner le mail de l'utilisateur !");
      return;
    }

    if(!item.roleId) {
      this.notificationService.showNotificationMessage("error","Veuillez sélectionner le profil de l'utilisateur !");
      return;
    }

    Swal.fire({
      title: (item && item?.id) ? "Modification utilisateur" : "Enregistrement utilisateur",
  ...
  saveItem(item : any) {
    ...
    const endPoint = "users/" + (item?.id ? "update" : "create");
