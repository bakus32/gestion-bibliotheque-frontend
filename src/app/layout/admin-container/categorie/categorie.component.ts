...
export class CategorieComponent implements OnInit {
  ...
  confirmSaveItem(item : any) {
    ...
    if(!item.libelle) {
      this.notificationService.showNotificationMessage("error","Veuillez renseigner le libelle de la categorie !");
      return;
    }


    Swal.fire({
      title: (item && item?.id) ? "Modification categorie" : "Enregistrement categorie",
  ...
  saveItem(item : any) {
    ...
    const endPoint = "categorieLivre/" + (item?.id ? "update" : "create");
  ...
  deleteItem(item : any) {
    ...
    const endPoint = "categorieLivre/delete";
